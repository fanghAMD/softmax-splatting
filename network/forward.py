...
class Flow(torch.nn.Module):
    ...
    class Decoder(torch.nn.Module):
      ...
      def forward(self, tenOne, tenTwo, objPrevious):
        ...
        if objPrevious is None:
          ...
          tenForward = (
            torch.nn.functional.interpolate(input=objPrevious["tenForward"], size=(intHeight, intWidth), mode="bilinear", align_corners=False)
  ...
  def forward(self, tenOne, tenTwo):
    ...
    return {
      "tenForward": torch.nn.functional.interpolate(input=objForward["tenForward"], size=(intHeight, intWidth), mode="bilinear", align_corners=False)
      * (float(intWidth) / float(objForward["tenForward"].shape[3])),
      "tenBackward": torch.nn.functional.interpolate(input=objBackward["tenForward"], size=(intHeight, intWidth), mode="bilinear", align_corners=False)
...
class Synthesis(torch.nn.Module):
  def __init__(self):
    ...
    class Softmetric(torch.nn.Module):
      ...
      def forward(self, tenEncone, tenEnctwo, tenFlow):
        ...
        write_png(f"intermediates/{self.dumpPath}/tenEncone_vis.png", _tenImg)

        _tenBackWarp = backwarp(tenEnctwo[0], tenFlow)
        write_png(f"intermediates/{self.dumpPath}/tenBackWarp_vis.png", _tenBackWarp[:, 0:3, :, :])
    ...
    class Warp(torch.nn.Module):
      ...
      def forward(self, tenEncone, tenEnctwo, tenMetricone, tenMetrictwo, tenForward, tenBackward):
        ...
        for intLevel in range(3):
          if intLevel != 0:
            ...
            tenForward = torch.nn.functional.interpolate(input=tenForward, size=(tenEncone[intLevel].shape[2], tenEncone[intLevel].shape[3]), mode="bilinear", align_corners=False) * (
              float(tenEncone[intLevel].shape[3]) / float(tenForward.shape[3])
            )
            tenBackward = torch.nn.functional.interpolate(input=tenBackward, size=(tenEnctwo[intLevel].shape[2], tenEnctwo[intLevel].shape[3]), mode="bilinear", align_corners=False) * (
      ...
      def frame_warp(self, tenOne, tenTwo, tenEncone, tenEnctwo, tenMetricone, tenMetrictwo, tenForward, tenBackward):
        ...
        def warp_mode(strMode):
          ...
          write_png(f"intermediates/{self.dumpPath}/{_strMode}_tenBackward_vis.png", visualize_flow(tenBackward))
  ...
  def forward(self, tenOne, tenTwo, tenForward, tenBackward, tenDepOne, tenDepTwo, fltTime):
    ...
    if tenDepOne is None and tenDepTwo is None:
      tenMetricone = self.netSoftmetric(tenEncone, tenEnctwo, tenForward) * 2.0
      tenMetrictwo = self.netSoftmetric(tenEnctwo, tenEncone, tenBackward) * 2.0
    else:
      # Game Engine Depth replaces Softmetric
      tenMetricone = tenDepOne * 2.0 * 10
      tenMetrictwo = tenDepTwo * 2.0 * 10

    if save_intermediates:
      self.netWarp.frame_warp(tenOne, tenTwo, tenEncone, tenEnctwo, tenMetricone, tenMetrictwo, tenForward, tenBackward)

    tenMetricone = tenMetricone * fltTime
    tenMetrictwo = tenMetrictwo * (1.0 - fltTime)
